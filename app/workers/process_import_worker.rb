class ProcessImportWorker
   ...
   sidekiq_options queue: :import, retry: true
   sidekiq_options backtrace: true
#   sidekiq_options unique: :until_executed,
#                   unique_args: ->(job) { [job.args[0], job.args[1]] }
    ...
    def perform(file)
      ...
      if @hash.present? && Item.where(id: @hash['ID']).count == 0
        ...
        #ProcessFilesWorker.perform_async(@hash['id'], files_hash) unless files_hash.empty?
      end  
    end
